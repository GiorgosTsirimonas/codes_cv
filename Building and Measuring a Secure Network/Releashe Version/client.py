...
#Function that downloads a file and prints the time it took to do so
...
#Function that given a public key, a signature and a hash validates the signature: returns true if it is valid , false otherwise
...
#Thread with the purpose of pinging an end server and displaying the Rtt
...
#Thread with the purpose of running a traceroute to an end server and displaying the number of hops
...
#Thread with the purpose of running a combination of pings and traceroute to and end server and displaying the average rtt and number of hops
...
#Thread with the purpose of communicating with a relay and signaling it to run a combination of pings and traceroute to an end server. The results returned are stored in its rtt and hops arrays
...
#Thread with the purpose of communicating with a relay and signaling it to run (ping || traceroute || combination) and then display the results
class relay_Thread (threading.Thread):
	...
	def run(self):
		...
		try:
			...
			if( not (private_key.can_encrypt() and private_key.can_sign() and private_key.has_private())):	#Check the key for errors
				print self.name + " An unexpected error occurred while generating private key"
				raise socket.timeout("")
				
			public_key = private_key.publickey()	#Generate public rsa cipher key
			
			
			thread_socket.connect( (relays_ip[self.iterator], int(relays_port[self.iterator])))	#Connect to a relay
			
			#print "Connection established with: " + relays[self.iterator]	#uncomment to show relay connections
...
#Thread with the purpose of pinging all available relays and either storing their rtt in an array or printing the results	
...
#Thread with the purpose of monitoring  other relay_latency_Thread and server_latency_Thread and after their are finished parses their results to derive the optimal relay for accessing an end server
class relay_optimal_path_Thread (threading.Thread):
	...
	def run(self):
		...
		for i in range (0, len(self.latency_thread_ar)):	#Wait for latency threads to finish
			latency_thread_ar[i].join()
		
		for i in range (0, len(self.relay_thread_ar)):	#Wait for relay threads to finish
		...
		for i in range (0, len(self.relay_rtt)):	#Add Rtt1 and Rtt2 (client->relay rtt and relay->end server rtt)
			rtt.append(-1)
		...
		fastest = -1
		optimal_position = -1
		for i in range (0, len(rtt)):	#Derive the optimal route to the end server
		...
		if optimal_position != -1:	#Display the optimal route to the end server
...
#Thread with the purpose of directly downloading a specified file
...
#Thread with the purpose of Signaling a relay for it to download a file, encrypt it and send it here
class relay_download_Thread (threading.Thread):
	...
	def run(self):
		...
		try:
			...
			thread_socket.connect( (relays_ip[self.iterator], int(relays_port[self.iterator])))	#Connect to relay
			...
			if response == "success":
				...
				cipher = AES.new(cipher_key, cipher_mode, cipher_blocksize)	#Generate AES cipher key
				
				decrypted_file_l = cipher.decrypt(buffer)[16:]	#Decrypt file using the cipher key
				
				relay_signature = decrypted_file_l[len(decrypted_file_l) - 128:]	#Parse the file removing the signature from it
				
				hash = SHA512.new(str(relays[self.iterator]))	#Generate a hash depending on the relay
				
				if not verify_sha512_signature(relay_public_key, relay_signature, hash):	#Verify the signature in the file
					print self.name + " File signature missmatch"
					raise socket.timeout("")
				
				
				decrypted_file = decrypted_file_l[:len(decrypted_file_l) - 128]	#Decrypt the file using the cipher key
				
				with open(self.extension, 'wb') as the_file:	#Write file to disk
					the_file.write(decrypted_file)
					#the_file.write(buffer)
				
				
				response = "\n" + self.name + " File '" + self.extension + "' downloaded successfully"	#Formating responses
				
			elif response == "failure":
				response = "\n" + self.name + " " + response	#Formating responses
			else:
				response = "\n" + self.name + " No response from relay"	#Formating responses
...
#Selects a mode R for relay mode or D for direct mode	
def select_mode():
...
endservers_filename = ''	#Filename of the end servers
relaynodes_filename = ''	#filename of the relay nodes
...
thread_counter = 0	#number of threads generated
random_generator = ''
...
while input != "exit":	#Entry to the state machine
	input = ""
